...
# Tracing is opt-in: without OTEL_EXPORTER_OTLP_ENDPOINT nothing is wired and
# no exporter ever dials out. Instrumented before the other middleware so the
# server span wraps the full request.
if os.environ.get("OTEL_EXPORTER_OTLP_ENDPOINT"):
    _tracer_provider = TracerProvider(
        resource=Resource.create(
            {"service.name": os.environ.get("OTEL_SERVICE_NAME", "url_shortener")}
        )
    )
