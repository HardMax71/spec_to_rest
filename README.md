...
# Generate a service AND a native conformance suite (pytest+Hypothesis / Vitest+fast-check / go test+rapid)
...
| `compile --with-tests --out <dir> <spec>`                          | Additionally emit a native behavioral/stateful/structural conformance suite (in the target's own language — pytest+Hypothesis+Schemathesis / Vitest+fast-check / `go test`+rapid), strategies, and a `/__test_admin__` router gated by `ENABLE_TEST_ADMIN`. See [test-generation.mdx](docs/content/docs/pipelines/test-generation.mdx). |
