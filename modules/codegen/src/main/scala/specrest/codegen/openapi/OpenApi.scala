...
object Schema:
  ...
  private def buildArraySchema(
    ...
    val items = if innerSchema.nullable then makeNullable(innerSchema.schema) else innerSchema.schema
    SchemaObject(
...
object Components:
  ...
  private final case class DecoratedField(name: String, schema: SchemaObject, nullable: Boolean)
...
object Paths:
  ...
  private def buildSuccessResponse(
    ...
    if status == 204 then ResponseObject("No content", None, None)
      ...
      jsonResponse("Successful response", SchemaObject(
        `type` = Some(List("array")),
      ...
      op.kind == OperationKind.Replace || op.kind == OperationKind.PartialUpdate then
      jsonResponse("Successful response", SchemaObject(
