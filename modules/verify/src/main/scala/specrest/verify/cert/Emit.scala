...
object Emit:
  ...
  def emit(ir: ServiceIR, proofsLeanPath: String): CertificateBundle =
    ...
    val (certified, stubbed) = theorems.foldLeft((0, 0)): (acc, t) =>
      val (c, s) = acc
      if t.contains("by sorry") then (c, s + 1) else (c + 1, s)
  ...
  private def renderHeader(ir: ServiceIR, moduleName: String): String =
    s"""import SpecRest.IR
       ...
       |
       |    Generated by `specrest.verify.cert.Emit`. Each theorem closes a
       |    closed-evaluation goal via `cert_decide` (= `native_decide`).
       |    Out-of-subset obligations are emitted as `sorry`-stubs with a
       |    `TODO[M_L.4]` marker pointing at the offending operator.
       |
  ...
  private def renderInvariantTheorem(
    ...
    VerifiedSubset.classify(inv.expr) match
      case VerifiedSubset.SubsetStatus.OutOfSubset(reason) =>
        renderSorryStub(theoremName, s"invariant `$displayName` for service `${ir.name}`", reason)
  ...
  private def renderSorryStub(
      theoremName: String,
      what: String,
      reason: String
  ): String =
    s"""/-- $what — OUT OF M_L.1 VERIFIED SUBSET: $reason. -/
       |theorem $theoremName : True := by sorry
       |
