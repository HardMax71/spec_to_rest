...
object GoStateful:
  ...
  private def renderModule(
    ...
    sb.append(s"import (\n$importBlock\n)\n\n")
    sb.append(s"func Test${goIdent(ir.a)}StatefulInvariants(t *testing.T) {\n")
  ...
  private def skipPlaceholder(ir: ServiceIRFull): String =
    s"""|//go:build conformance
        ...
        |//
        |// No native stateful test could be built for ${ir.a}: every operation is
        ...
        |import "testing"
        |
        |func Test${goIdent(ir.a)}StatefulAllSkipped(t *testing.T) {
