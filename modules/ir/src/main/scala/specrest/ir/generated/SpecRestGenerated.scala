...
object SpecRestGenerated {
  ...
  def equal_ir_valuea(x0: ir_value, x1: ir_value): Boolean = (x0, x1) match {
    ...
    case (VNone(), VEntity(x51, x52))                    => false
    ...
    case (VNone(), VEnum(x41, x42))                      => false
  ...
  final case class Matches(a: expr, b: String, c: Option[span_t])    extends expr
  ...
  final case class MapCons(a: expr, b: expr, c: expr, d: Option[span_t])
      extends expr
  final case class UStrPred(a: String, b: expr, c: Option[span_t]) extends expr
  ...
  def dom_arg(x0: expr_full): Option[String] = x0 match {
  ...
  def lower_dom_eq(xrel: String, yrel: String, sp: Option[span_t]): expr =
    BoolBin(
      AndOp(),
      ForallRel("0cmp", xrel, Member(Ident("0cmp", None), yrel, sp), sp),
      ForallRel("0cmp", yrel, Member(Ident("0cmp", None), xrel, sp), sp),
      sp
    )

  def lower_beq_dom_or_none(l: expr_full, r: expr_full, sp: Option[span_t]): Option[expr] =
    (dom_arg(l), dom_arg(r)) match {
      case (None, _)          => None
      case (Some(_), None)    => None
      case (Some(x), Some(y)) => Some[expr](lower_dom_eq(x, y, sp))
    }
  ...
  def lower_with_assigns(
    ...
    (vl, x1, base, vm) match {
      case (vl, Nil, base, vm) => Some[expr](base)
      case (enums, FieldAssignFull(fld, v, vn) :: rest, base, sp) =>
        lower(enums, v) match {
          case None => None
          case Some(va) =>
  ...
  def lowerMapEntries(
    ...
    (vp, x1, sp) match {
      case (vp, Nil, sp) => Some[expr](MapEmpty(sp))
      case (enums, MapEntryFull(k, v, vq) :: rest, sp) =>
  ...
  def lowerSetList(vk: List[String], x1: List[expr_full], sp: Option[span_t]): Option[expr] =
    (vk, x1, sp) match {
      case (vk, Nil, sp) => Some[expr](SetEmpty(sp))
  ...
  def lowerSeqList(vo: List[String], x1: List[expr_full], sp: Option[span_t]): Option[expr] =
    (vo, x1, sp) match {
      case (vo, Nil, sp) => Some[expr](SeqEmpty(sp))
      case (enums, e :: rest, sp) =>
  ...
  def lower(uu: List[String], x1: expr_full): Option[expr] = (uu, x1) match {
    ...
    case (enums, CallF(callee, args, sp)) =>
      (callee, args) match {
        case (BinaryOpF(_, _, _, _), _)         => None
        case (UnaryOpF(_, _, _), _)             => None
        case (QuantifierF(_, _, _, _), _)       => None
        ...
        case (IdentifierF(nm, _), List(arg)) =>
          is_builtin_pred(nm) match {
            case true  => map_option[expr, expr]((a: expr) => UStrPred(nm, a, sp), lower(enums, arg))
            case false => None
          }
    ...
    case (ve, SetComprehensionF(vf, vg, vh, vi)) => None
      ...
      dm match {
        ...
        case IdentifierF(rel, _) =>
          string_in_list(rel, enums) match {
            case true => None
            case false => map_option[expr, expr](
                (b: expr) =>
                  TheRel(vara, rel, b, sp),
                lower(enums, body)
              )
          }
      }
    case (enums, MatchesF(e, pat, sp)) =>
      map_option[expr, expr]((ea: expr) => Matches(ea, pat, sp), lower(enums, e))
    ...
    case (enums, BinaryOpF(op, l, r, sp)) =>
      op match {
        ...
        case BEq() =>
          lower_beq_dom_or_none(l, r, sp) match {
            case None =>
              r match {
    ...
    case (vj, EnumAccessF(base, mem, sp)) =>
      base match {
        case BinaryOpF(_, _, _, _)         => None
        case UnaryOpF(_, _, _)             => None
        case QuantifierF(_, _, _, _)       => None
    ...
    case (enums, SomeWrapF(e, sp)) =>
      map_option[expr, expr]((ea: expr) => SomeE(ea, sp), lower(enums, e))
  }
  ...
  def eval_the_rel(
      ...
      x4: List[ir_value],
      body: expr
  ): Option[List[ir_value]] =
    (s, st, env, vara, x4, body) match {
      case (s, st, env, vara, Nil, body) => Some[List[ir_value]](Nil)
      case (s, st, env, vara, v :: rest, body) =>
        eval(s, st, (vara, v) :: env, body) match {
          case None => None
          case Some(VBool(b)) =>
            eval_the_rel(s, st, env, vara, rest, body) match {
              case None => None
              case Some(matches) =>
                Some[List[ir_value]](b match {
                  case true  => v :: matches
                  case false => matches
                })
            }
  ...
  def eval(
    ...
    (s, st, env, x3) match {
      ...
      case (s, st, env, TheRel(vara, rel_name, body, vj)) =>
        state_relation_domain(st, rel_name) match {
          case None => None
          case Some(rel_dom) =>
      ...
      case (s, st, env, Matches(e, pat, vz)) =>
        eval(s, st, env, e) match {
          ...
          case Some(VNone())              => None
          case Some(VSome(_))             => None
          case Some(VStr(str))            => Some[ir_value](VBool(str == pat))
          case Some(VSeq(_))              => None
          case Some(VMap(_))              => None
        }
      ...
      case (s, st, env, UStrPred(name, e, we)) =>
        eval(s, st, env, e) match {
          case None                       => None
          case Some(VBool(_))             => None
          case Some(VInt(_))              => None
          ...
          case Some(VNone())              => None
          case Some(VSome(_))             => None
          case Some(VStr(str))            => Some[ir_value](VBool(name == str))
          case Some(VSeq(_))              => None
          case Some(VMap(_))              => None
        }
    }
  ...
  def translate(x0: expr): smt_term = x0 match {
    ...
    case UStrPred(name, e, wn) => TUStrPred(name, translate(e))
    case SeqEmpty(wo)          => TSeqEmpty()
    case SeqCons(e, rest, wp)  => TSeqCons(translate(e), translate(rest))
    case MapEmpty(wq)          => TMapEmpty()
    case MapCons(k, v, rest, wr) =>
  ...
  def identNameFull(x0: expr_full): Option[String] = x0 match {
  ...
  def foldTrust(enums: List[String], exprs: List[expr_full]): trust_level =
    list_all[expr_full](
      (e: expr_full) =>
        !is_none[smt_term](translate_full_direct(enums, e)),
      exprs
    ) match {
      case true  => TlSound()
  ...
  def peelRelationRefFull(x0: expr_full): Option[String] = x0 match {
    case IdentifierF(rel, uu)             => Some[String](rel)
    case PreF(b, uv)                      => identNameFull(b)
    case PrimeF(b, uw)                    => identNameFull(b)
    case BinaryOpF(v, va, vb, vc)         => None
    case UnaryOpF(v, va, vb)              => None
    case QuantifierF(v, va, vb, vc)       => None
    case SomeWrapF(v, va)                 => None
  ...
  def translate_full(enums: List[String], e: expr_full): Option[smt_term] =
    map_option[expr, smt_term]((a: expr) => translate(a), lower(enums, e))
