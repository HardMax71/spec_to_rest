...
clauses that emit a skip do so only because of *user errors* in the spec (unbound
identifier, reserved-name binding, call to a function that's neither a built-in nor
declared via `function`/`predicate`) or `Quantifier` typed-bindings (`Colon`), which need
type-name → instance enumeration not yet wired. Skipped clauses are recorded in
`tests/_testgen_skips.json` and omitted from the generated tests rather than emitted as
`pytest.skip` notes.
