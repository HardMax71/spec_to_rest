...
was chosen)? See the [spec language foundations](/research/spec_language_design/survey).

The semantics on this page are English-prose. The matching formal denotational semantics
ships in [`Semantics_Reference.thy`](https://github.com/HardMax71/spec_to_rest/blob/main/proofs/isabelle/SpecRest/semantics/Semantics_Reference.thy)
(an `eval` mutual function block over the IR ADT), and the universal translator-soundness
theorem (`translate_soundness_standalone`) closes in
[`DirectSound.thy`](https://github.com/HardMax71/spec_to_rest/blob/main/proofs/isabelle/SpecRest/soundness/DirectSound.thy)
with zero `sorry` over the verified subset. Production `verify` routes in-subset
...
[Mechanically Verified Translator Soundness](/research/translator_soundness/theorem);
