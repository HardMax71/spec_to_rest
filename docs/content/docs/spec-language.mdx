...
translates it to a Python assertion. The translator (M5.1) handles a working subset of the
`Expr` ADT; clauses that use shapes outside the subset are recorded in
`tests/_testgen_skips.json` and are omitted from the generated tests rather than emitted
as `pytest.skip` notes.

| Translated | Skipped (deferred to [M5.5](https://github.com/HardMax71/spec_to_rest/issues/133)) |
|---|---|
| literals, `Identifier`, `BinaryOp` (all shapes), `UnaryOp` (Not / Negate / Cardinality) | `Power` |
| `Pre(...)`, `Prime(...)`, `FieldAccess`, `Index`, `EnumAccess` | — |
| `If`, `Let`, `SetLiteral`, `Quantifier` (All/Some/Exists/No, `in`-bindings) | `Quantifier` with type-bindings (Colon) |
| recognized calls: `len`, `dom`, `ran`, `isValidURI`, `valid_email` | unknown user-defined `Call` |
| bare enum-member identifiers resolve to string literals | — |
| | `MapLiteral`, `Constructor`, `With` (record update), `SetComprehension`, `SeqLiteral`, `Matches`, `The`, `Lambda`, `SomeWrap` |

Empirical skip rates on the canonical fixtures (locked in CI): `safe_counter` 0.0%,
`url_shortener` 9.5%, `todo_list` 16.0%. See
