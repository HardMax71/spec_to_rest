...
As a rule of thumb, if your `fun` body never appears as a `using foo.simps`,
...
| "Apply this specific lemma after some rewriting"      | `by (metis lemma1 lemma2)`, 2 to 5 explicit facts     |
...
quickly becomes 100 to 200 generated arms even when the Isabelle source
is two lines.
...
As a rule of thumb, before committing a lift, run the regen pipeline
...
Cap the wait at 5 to 6 minutes per iteration. Anything longer and you're
...
We keep symbolic name refs deliberately. MLIR keeps its
