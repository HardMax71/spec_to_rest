...
- An `Option<T>` datatype + per-spec `datatype` declarations for enums and
  entities.
- Type aliases with their `where` constraints lifted to a parallel
  `<T>Where(value)` predicate (entity invariants likewise become
  `<T>Inv(x)`).
- A `class ServiceState { var ... }` mirroring the spec's `state { ... }`
  block, with each invariant clause rolled into a `ServiceStateInv(st)`
  predicate.
...
- Calls used in *value* position (e.g. `now()` returning a timestamp) emit as
  ...
  is a follow-up.
- Set comprehensions over relations may emit a syntactically valid but
  semantically lossy iteration (key vs. value projection).
