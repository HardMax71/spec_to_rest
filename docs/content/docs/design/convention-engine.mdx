...
see [Convention Engine](/research/convention_engine/ruleset).
...
[Convention Engine section M6](/research/convention_engine/ruleset) for the planned semantics.
...
Derived or overridden paths must stay clear of the generated infrastructure
surface: `check` and `compile` reject any operation whose route resolves to
`/health`, `/ready`, `/metrics`, or into the `/admin` prefix
(`Validate.validateRoutes`). Rename the operation or point its `http_path`
somewhere else.
...
Profiles bundle target-language and runtime conventions (types, ORM column mapping, framework
imports, and so on). The registry ships three frameworks, each bound to one language: FastAPI
(Python, on SQLAlchemy 2 async and Alembic), Chi (Go), and Express (TypeScript, on Prisma). Every
framework supports all three SQL dialects: PostgreSQL, SQLite, and MySQL.

A target is the slug `<language>-<framework>-<database>`, for example `python-fastapi-postgres`
(the default), `go-chi-sqlite`, or `ts-express-mysql`. `compile` accepts any registered
combination; the `--framework`, `--db`, and `--lang` flags resolve to a slug, and a combination the
registry does not support is rejected at the command line before generation starts.

Multiple-environment overrides (development / staging / production / testing) remain roadmap work.

The registry is a small `Map[String, Framework]` in
...
adding a target is a framework entry plus its Handlebars template tree.
