...
modules/
  ...
  verify/     — Translator, SmtLib, Backend (Java Z3), Alloy backend, Consistency,
                CounterExample, Diagnostic — every public entry point returns IO
...
`parTraverseN(n)`; `--timeout` maps to `IO.timeoutTo` plus solver-native deadlines. See
