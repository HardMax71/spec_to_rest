...
| Default `--with-tests` after M5.4 | **shipped** — [M5.12 (#140)](https://github.com/HardMax71/spec_to_rest/issues/140); test emission is on by default for every supported target, with `--no-tests` as the opt-out (mutually exclusive with `--with-tests`) |
