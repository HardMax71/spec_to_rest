...
| 4     | Verification (Z3 + Alloy + Isabelle/HOL universal soundness)   | **Shipped.** Pivoted from Lean 4 to Isabelle/HOL via [#193](https://github.com/HardMax71/spec_to_rest/issues/193); IR canonicalised in [#202](https://github.com/HardMax71/spec_to_rest/issues/202); production verify routes in-subset checks through the Isabelle-extracted translator ([#192](https://github.com/HardMax71/spec_to_rest/issues/192)). See [Verification capabilities](#verification-capabilities) below. |
...
| 7     | Multi-target codegen — Go/chi, TS/Express                      | **Not started.** [#33](https://github.com/HardMax71/spec_to_rest/issues/33), [#34](https://github.com/HardMax71/spec_to_rest/issues/34) (distribution), [#35](https://github.com/HardMax71/spec_to_rest/issues/35), [#36](https://github.com/HardMax71/spec_to_rest/issues/36) (CLI polish). |
...
| M_L.3 ([#129](https://github.com/HardMax71/spec_to_rest/issues/129)) | Per-run certificate emission | shipped, then **deleted post-pivot** — universal soundness made it vestigial |
...
| IR canonicalisation ([#202](https://github.com/HardMax71/spec_to_rest/issues/202)) | Full input-language IR in Isabelle; `lower :: expr_full ⇒ expr option` projection; flat case-class extraction | shipped |
...
- **Docs CLI snippets** ([#232](https://github.com/HardMax71/spec_to_rest/pull/232)) — Pydantic-style `<CliRun>` panels in MDX with build-time CLI execution + drift gate.
...
| Temporal `always(P)` — `P` holds in every state satisfying the invariants | shipped (Alloy, bounded scope; implemented as `I ∧ ¬P` unsat-check). Runtime artifacts: `temporal_always_<name>` per-op behavioral test + `@invariant()` block in stateful test + `x-temporal` OpenAPI extension (see [Test Generation](/pipelines/test-generation) §4) |
| Temporal `eventually(P)` — some state satisfies `P` and the invariants | shipped (Alloy, bounded scope; implemented as `I ∧ P` sat-check). Runtime artifacts: stateful state machine emits a `_eventually_seen_<name>` flag in `_reset`, an `@invariant()` observer that flips it when `P` holds, and a `teardown(self)` method that asserts the flag (see [Test Generation](/pipelines/test-generation) §4). Strictly weaker than the Alloy proof — bounded by per-case trace length |
...
- **L2 operation decomposition for graduated fallback** — [#227](https://github.com/HardMax71/spec_to_rest/issues/227), closed 2026-05-09 wontfix-with-pivot. Replaced by M6.7 hint-augmentation per the literature analysis in [research/12](/research/12_compositional_synthesis_findings) (DafnyComp 7% ceiling, Pass@4 plateau, +16pp from hint augmentation in DafnyPro POPL 2026).
