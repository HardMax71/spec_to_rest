...
| 7     | Multi-target codegen, Go/chi, TS/Express                      | **In progress.** Go/chi target shipped in [#33](https://github.com/HardMax71/spec_to_rest/issues/33); see [go-chi-postgres](/targets/go-chi-postgres). TS/Express target shipped in [#35](https://github.com/HardMax71/spec_to_rest/issues/35); see [ts-express-postgres](/targets/ts-express-postgres). CLI polish shipped in [#36](https://github.com/HardMax71/spec_to_rest/issues/36): `compile --dry-run`, `diff`, `test`, `--color/--no-color`, exit-code reference — see [CLI Reference](/cli). Distribution shipped in [#34](https://github.com/HardMax71/spec_to_rest/issues/34): 4-platform release binaries (linux-amd64, macos-arm64, macos-amd64, windows-amd64), GHCR Docker image (`ghcr.io/hardmax71/spec-to-rest`), and a composite GitHub Action — see [Install](/install). |
