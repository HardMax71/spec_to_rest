...
| `modules/verify/src/main/scala/specrest/verify/cert/EvalIR.scala` | Proof-owned core | `tracked` | Scala-side reducer mirroring `proofs/lean/SpecRest/Semantics.lean#eval` over the verified subset. Drift is detected at `lake build` time of emitted certs; this is the M_L.3 translation-validation cross-check. |
...
| `.github/workflows/lean-certs.yml` | Proof-owned CI | `tracked` | M_L.3 sidecar matrix: per fixture, run `verify --emit-cert` + `lake build` against the cert bundle. Off the required-checks list (sidecar, like `lean.yml`). Matrix expanded in M_L.4.a-d era to 6 fixtures (safe_counter, set_ops, auth_service, url_shortener, todo_list, edge_cases). |
...
| `modules/verify/src/test/scala/specrest/verify/audit/ProofDriftAuditTest.scala` | Drift-control artifact | `tracked` | 6-check Tier-A drift suite (A1 shape coverage, A2 translator-literal presence, A3 theorem-stem agreement, A4 InSubset never UNRENDERABLE, A5 Lean ⊆ Scala enum cases, A7 `Cert.lean` SHA-256 fingerprint). Required in PR CI. |
| `modules/verify/src/test/scala/specrest/verify/audit/CanonicalProbes.scala` | Drift-control artifact | `tracked` | Canonical `Expr` probes for every shape (in-subset, deferred, excluded). Adding a new `Expr` case to `Types.scala` requires a probe entry here. |
| `proofs/lean/.cert-sha` | Drift-control artifact | `tracked` | SHA-256 fingerprint of `Cert.lean`. Drift suite A7 fails if `Cert.lean` changes without an explicit fingerprint bump. Forces reviewer-approval on tactic-library changes that affect the trust closure of every emitted cert. |
| `proofs/lean/.last-release-sha` | Drift-control artifact | `tracked` | Commit SHA at which `proofs/lean/lakefile.toml` version was last bumped. Anchor for future A8 lakefile-monotonicity check. |
