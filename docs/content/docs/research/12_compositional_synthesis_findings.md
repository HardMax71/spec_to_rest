...
description: Why we pivoted M6.7 from operation decomposition to hint-augmentation, with empirical evidence from DafnyComp, Re:Form, and DafnyPro plus the failure-mode analysis that informed the decision.
---
...
- Sampling more candidates does **not** rescue the result, Pass@4 → Pass@8 yields +0.8% on average. The plateau is architectural, rather than search-budget bound.
...
A **14.4× verification-rate drop for a 3.2× function-count increase**. The compositional axis dominates everything else (model choice, sampling budget, reasoning specialization) by a wide margin.
...
The current state of the art for compositional Dafny verification, and it requires RL training, rather than inference-time tricks.
...
1. **No inference-time technique reaches this ceiling.** Our orchestrator is inference-only; we don't train models. So 14% is a north star, rather than a budget.
...
- M6.6 L1 (prompt strategies) + L3 (model escalation), small marginal gains; per the DafnyComp Pass@k plateau, rather than transformative.
