...
each scope under them: a relation an invariant reads outside its own rows
loads whole whenever a referencing relation hydrates non-empty, and clause
shapes the classifier cannot vouch for force their relations whole on every
request. The one trade left is the guard's reach: a stored invariant
violation among rows a request never loads (possible only through
out-of-band writes such as admin seeding) is caught by the next request
that hydrates them, not by every request.
