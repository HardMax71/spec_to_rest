...
satisfies the requires, and a partially hydrated state is such a state. The
one trade is the runtime guard: `ServiceStateInv` evaluates over the
hydrated slice, so a stored invariant violation among rows a request never
