...
Z3's native `solver.check()` does **not** observe `Thread.isInterrupted`, and because the call
runs inside `IO.blocking` we wouldn't send an interrupt anyway. On cancel, the solver thread
continues until it honours its own inner `timeout` parameter (set via `--timeout`), then
`Resource` finalizers close the `Context`. Practical bound: worst-case native CPU after cancel
is `cfg.timeoutMs + solver_slack`, typically well under `2 × cfg.timeoutMs`. True mid-call
abort via `Context.interrupt()` is tracked separately in
