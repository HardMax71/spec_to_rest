...
functions to ~1.4 kLoC of idiomatic Scala 3 (BigInt-mapped) at
`modules/verify/src/main/scala/specrest/verify/cert/generated/SpecRestGenerated.scala`. The
Scala layer's `translate` is no longer hand-written — it is the extracted Isabelle definition.
