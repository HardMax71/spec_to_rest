...
- [**Alloy 6**](https://alloytools.org/) (in-process via `org.alloytools:org.alloytools.alloy.core`)
  — handles checks whose invariant uses second-order constructs (powerset `^s`, relational
  closures). Bounded-scope search only; scope 5 by default, tunable via `--alloy-scope`.
...
def classifyInvariant(inv: InvariantDecl): VerifierTool =
  if inv.expr contains powerset (^s) or transitive closure
