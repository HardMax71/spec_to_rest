...
<FileTree>
  <FileTreeFolder name="out/">
    <FileTreeRow name="global.smt2" note="combined satisfiability of all invariants" />
    <FileTreeRow name="Shorten.requires.smt2" note="precondition feasibility for operation Shorten" />
    <FileTreeRow name="Shorten.preserves.clickCountNonNegative.smt2" note="preservation VC: op × invariant" />
    <FileTreeRow name="Delete.requires.smt2" note="precondition feasibility for operation Delete" />
    <FileTreeRow name="temporal.alwaysSafe.als" note="Alloy-routed temporal check" />
    <FileTreeRow name="verdicts.json" note="{ id, tool, outcome, rawStatus, durationMs, file } per check" />
  </FileTreeFolder>
</FileTree>
...
`--timeout` maps to a single solver-native deadline on each backend:

- **Z3**: the `timeout` solver parameter (and the SMT-LIB `(set-option :timeout …)` the
  emitter renders for `--dump-smt`).
- **Alloy**: `Future.get(timeout, MILLISECONDS)` on the Kodkod solver thread; on
  `TimeoutException` the backend calls `future.cancel(true)` and the Kodkod thread stops.

There is no outer `IO.timeoutTo` wrapper in `Consistency.runConsistencyChecks`. If a
misbehaving solver ignores its own inner deadline, the check holds its fiber until natural
completion — practical worst-case native CPU is `cfg.timeoutMs + solver_slack`, well under
`2 × cfg.timeoutMs` in real specs.

`0` disables the solver-native deadline (standard Z3 "no timeout" semantics).
