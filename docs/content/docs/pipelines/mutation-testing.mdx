...
- **Phase 6 (#27–#32) — handler synthesis from `ensures` clauses.** This is
  ...
  [#30](https://github.com/HardMax71/spec_to_rest/issues/30) /
  [#32](https://github.com/HardMax71/spec_to_rest/issues/32). The first wedge,
  [#31 — operation classification (Direct Emit vs LLM)](https://github.com/HardMax71/spec_to_rest/issues/31),
  has shipped: `Classify` now stamps each operation with a `SynthesisStrategy`
  and `inspect` surfaces it; downstream Phase 6 issues will gate their LLM
  calls on that field.
