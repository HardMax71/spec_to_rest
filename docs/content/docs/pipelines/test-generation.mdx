...
*implementation* is stack-specific (`AdminRouter.scala` → FastAPI
`app/routers/admin.py`, `AdminRouterTs.scala` → Express, `AdminRouterGo.scala` → Go).
...
non-transition ops, `GuardSatisfier` extensions, M5.11 multi-target test gen (both shipped) and the historical M5.12
default-on test emission), see [Roadmap $\to$ Test generation follow-ups](/roadmap#test-generation-follow-ups).
