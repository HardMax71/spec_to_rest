...
Resolution: `<Operation>.test_strategy.<input>` wins over `<Entity>.test_strategy.<field>`,
which broadcasts to every operation whose input shares the field name. The validator
rejects unknown field qualifiers, missing qualifiers, and values other than
`"live"` / `"redacted"`. Convention rules with the same target+qualifier+property are
duplicate errors.
