...
`ExprToPython` covers every `Expr` constructor in the IR. The remaining `ExprPy.Skip`
outputs all signal *user errors* (unbound identifier, reserved-name binding, spec call to
a function that's neither a built-in nor declared in the spec) rather than translator
gaps. These numbers are locked in `SkipRateProbeTest.scala`; CI fails if they regress.
