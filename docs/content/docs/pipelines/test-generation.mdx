...
description: compile emits a native behavioral/stateful/structural conformance suite (per target language) + a gated admin router from spec ensures/requires/invariants
---
...
`main.go` wiring, and the default-on test emission (opt out with `--no-tests`) adds the
real `conformance`-tagged router — keep the test-admin code out of production binaries
via the build tag, not a runtime branch.
