...
[EBNF specification](/research/spec_language_design/grammar) and the reasoning behind each
deviation.

## Keyword-as-identifier problem

The EBNF assumes identifiers and keywords are context-free: `output` in `output.url` is just
an identifier.

ANTLR4's lexer is context-independent. Once `output` is defined as the `OUTPUT`
...
The fix is a `lowerIdent` parser rule that accepts `LOWER_IDENT` _and_ all keywords that may
...
The cost is that every keyword becomes a valid identifier, which is overly permissive at the
grammar level. Semantic analysis (a later phase) should reject nonsensical uses like naming a
field `service`.

## `set` multiplicity vs `Set[T]` type

In the EBNF, `set` (lowercase) is a multiplicity keyword and `Set` (uppercase) is the
parameterized collection type. The EBNF treats them as separate tokens implicitly.

In ANTLR4, the `SET` keyword is defined as `'Set'` (uppercase S). Lowercase `set` in
`Int -> set String` gets lexed as `LOWER_IDENT`, which doesn't match the `multiplicity` rule.

The fix is a separate lexer token `SET_MULT: 'set'` for the lowercase multiplicity, distinct
...
The EBNF spec writes `expr_list = expr { NEWLINE expr }`: newlines are significant separators between
top-level expressions in `requires:` and `ensures:` blocks.

In the ANTLR4 implementation, all whitespace (including newlines) goes to `channel(HIDDEN)`. The
blocks use `expr+` instead of newline-delimited lists.

This works because expressions in this DSL are self-delimiting; there is no juxtaposition or
...
The risk: if the DSL ever adds a juxtaposition operator (e.g., implicit function application),
...
The EBNF spec defines both `REGEX_LIT` (`/pattern/`) and division (`/`). This creates a
classic lexer ambiguity: `a / b / c` could be tokenized as division or as a regex literal.

The fix is a lexer predicate that restricts `REGEX_LIT` to match only immediately after the
...
The EBNF approach uses 10+ separate named rules (`or_expr`, `and_expr`, `not_expr`, etc.) forming a
precedence chain.

The ANTLR4 approach uses a single left-recursive `expr` rule with labeled alternatives. ANTLR4
assigns implicit precedence by alternative order: the first alternative binds tightest (highest
precedence), the last binds loosest. The alternatives are ordered accordingly:
...
An early mistake: the first version of the grammar had the order inverted (or first, postfix
...
This is more concise than the EBNF chain and uses ANTLR4's native left-recursion elimination.

## `some` disambiguation

The ambiguity is `some(expr)` (Option wrapper) vs `some x in S | P` (existential quantifier).

The EBNF uses separate rules `some_wrap_expr` and `quantifier_expr`.

In ANTLR4, the `someWrapExpr` sub-rule and the `quantifierExpr` sub-rule are both
