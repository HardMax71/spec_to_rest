...
`spec-to-rest compile` emits nothing when any check fails.
...
and an excerpt of the FastAPI router the emitter writes, all from one
`compile` invocation:
...
mechanically discharged. The guarantee is concrete: on every input the
specification admits, the generated handlers leave every invariant intact.
...
When `verify` succeeds, `compile` emits the full project: `app/`, `alembic/`,
`tests/`, `Dockerfile`, `docker-compose.yml`, `openapi.yaml`, and a
GitHub Actions workflow. When `verify` fails, each failing check is rendered
in prose: which operation, which invariant, which contributing field, with
concrete pre and post values lifted from the solver model.
