...
`windows-amd64` (`.zip` archive). The binary is a single static executable
(~80 MB) with no JVM required at runtime; cold start is ~50 ms.
...
runs as a non-root `spec` user, and sets `WORKDIR /workspace`. Bundles glibc +
ca-certificates only — for `compile --with-synthesis` (needs Dafny) or
`test` (needs Python) install those on the host and use the native binary
instead.
...
the natural distribution is a single static binary — exposing it through npm
would mean shipping a wrapper package that just downloads the same archive
this action does at install time. If you are driving CI from `package.json`,
