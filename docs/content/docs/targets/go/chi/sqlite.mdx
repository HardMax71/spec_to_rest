...
container; the database is a local file. `internal/database/database.go` opens it
with `sql.Open(sqliteshim.ShimName, dsn)` and wraps it in
`bun.NewDB(sqldb, sqlitedialect.New())`.

golang-migrate's SQLite driver runs each migration in its own transaction, so the
emitted `migrations/*.sql` omit the explicit `BEGIN;`/`COMMIT;` the Postgres
target uses (a nested transaction would error).

SQLite only auto-increments the `INTEGER PRIMARY KEY` rowid alias, so the PK is
declared inline (no separate `CONSTRAINT pk_…`).

The column types are `String→TEXT`, `Int→INTEGER`, `Float→REAL`, `Bool→BOOLEAN`,
`DateTime→DATETIME`, `Date→DATE`, `UUID→TEXT`, `Decimal→NUMERIC`, `Bytes→BLOB`,
and `Set/Seq→TEXT`. The emitter is canonical.
