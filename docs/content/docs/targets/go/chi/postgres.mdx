...
dialect** (postgres, sqlite, mysql). It is **not** available for `go-chi-*`: testgen is a
Python/pytest + Hypothesis emitter that drives a FastAPI app and depends on the FastAPI-only
`/__test_admin__/*` reset router — there is simply no Go test emitter yet. `go-chi-*` emits only the
smoke `tests/health_test.go`. This is missing implementation, not a principled boundary; the
behavioral/stateful layers are already pure black-box HTTP, so a language-agnostic runner is
feasible. Tracked in [#265](https://github.com/HardMax71/spec_to_rest/issues/265).
