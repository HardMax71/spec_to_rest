...
`modules/codegen/src/main/resources/templates/python-fastapi-postgres/`.

**Database dialects ([#58](https://github.com/HardMax71/spec_to_rest/issues/58)).** The same Python
templates also back the `python-fastapi-sqlite` and `python-fastapi-mysql` profiles. The database
axis is a pluggable `Dialect` strategy (`modules/codegen/.../migration/Dialect.scala`): it decides
the SQLAlchemy column type, trigger shape, partial-index handling, and the connection / docker-compose
wiring (`DialectView`). SQLite is file-backed (no compose DB service, `PRAGMA foreign_keys=ON`
listener); MySQL uses a `mysql:8.4` service and `VARCHAR(255)` for unbounded strings. Adding another
dialect is one `Dialect` implementation plus a profile — no emitter or template changes.
