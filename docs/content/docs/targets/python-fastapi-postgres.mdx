...
The interactive view below is rendered from `docs/public/openapi/url_shortener.yaml`, a snapshot
of the actual emitter output for `fixtures/spec/url_shortener.spec`. Regenerate with:

```bash
sbt "cli/run compile --target python-fastapi-postgres --ignore-verify --out /tmp/openapi-gen fixtures/spec/url_shortener.spec"
