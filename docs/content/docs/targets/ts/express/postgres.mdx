...
Conformance / property / stateful test generation (`--with-tests`) is available for **every fastapi
dialect** (postgres, sqlite, mysql). It is **not** available for `ts-express-*`: testgen is a
Python/pytest + Hypothesis emitter that drives a FastAPI app and depends on the FastAPI-only
`/__test_admin__/*` reset router — there is simply no TypeScript test emitter yet. `ts-express-*`
emits only the smoke `tests/health.test.ts`. This is missing implementation, not a principled
boundary; the behavioral/stateful layers are already pure black-box HTTP, so a language-agnostic
runner is feasible. Tracked in [#265](https://github.com/HardMax71/spec_to_rest/issues/265).
