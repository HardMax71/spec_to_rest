...
- **Compose / CI service.** The emitted `docker-compose.yml` and
  ...
  Postgres one).
- **No native type attributes.** The generated `schema.prisma` models omit the
  Postgres-only `@db.*` native types; Prisma's MySQL provider defaults are valid
  and the actual column types come from the hand-rendered, dialect-aware
  `migration.sql`.
- **Column types.** `String→VARCHAR(255)`, `Int→BIGINT`, `Float→DOUBLE`,
  `Bool→TINYINT(1)`, `DateTime→DATETIME`, `Date→DATE`, `UUID→CHAR(36)`,
  `Decimal→DECIMAL`, `Bytes→LONGBLOB`. A 32-bit `SERIAL` PK stays
