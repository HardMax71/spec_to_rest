...
- The emitted `docker-compose.yml` provisions a `mysql:8.4` database service
...
- Unbounded string fields map to `VARCHAR(255)` rather than Postgres `TEXT`,
  since MySQL cannot index unbounded text without a key length.
- Remaining Postgres-specific types are mapped to their MySQL equivalents by the
