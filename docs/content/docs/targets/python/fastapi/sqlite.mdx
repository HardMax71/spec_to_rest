...
SQLite. The emitted suite itself is **dialect-invariant** — byte-identical across every fastapi
dialect, asserted deterministically by `TestEmitTest`. (A fully green conformance _run_ additionally
needs `--with-synthesis` to fill non-CRUD operation bodies; that is independent of the database
axis.)
