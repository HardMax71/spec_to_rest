...
text \<open>Productionized Scala extraction. The full-language IR (\<open>expr\<close>,
  ...
  Reference-semantics records (\<open>state\<close>, \<open>smt_model\<close> where reached, …) extract as
  polymorphic scheme types but Scala consumers don't reference them: \<open>translate\<close>
  outputs the flat \<open>smt_term\<close> datatype. The \<open>eval\<close>/\<open>smtEval\<close> reference
  interpreters are proof-internal and deliberately NOT exported: they have no
  Scala callers, and their subtree (abstract-const stubs included) was the
  largest consumer-dead slice of the artifact.\<close>
