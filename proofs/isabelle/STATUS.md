...
| 6     | **shipped (MVP)** | `EmitIsabelle.scala` ships parallel to `cert/Emit.scala`. Renders 14 of the 23 in-subset `Expr` shapes (atoms, unary, BoolBin/Arith/Cmp, Member, EnumAccess, Quantifiers, LetIn, Prime/Pre, CardRel/IndexRel/FieldAccess, SetEmpty) to Isabelle `expr` syntax with `(STR ''…'')` String.literal escaping. `emit(ServiceIR)` produces a `theory <name>_Cert imports SpecRest.Codegen begin … end` bundle. 10/10 unit tests passing. |
| 7     | **shipped (MVP)** | `A8RoundTripOracleTest` exercises every in-subset Expr shape end-to-end: Scala IR → `EmitIsabelle.renderExpr` (Isabelle source) AND `toExtracted` (extracted Scala expr) → `SpecRestGenerated.translate` → SmtTerm. 24/24 canonical probes pass (1 out-of-subset skip is honest); `verify/test` 219/219 tests passing.                                                                                                             |
